frapp.factory('Delete', function () {
    ...
    Delete.deleteProgramNode = (e, layer, stage) => {
        ...
        for (let each of node.customClass.execInPins) {
            ...
            for (let i = 0; i < len; i++) {
                if (each.wire[0]) { Delete.deleteWire(each.wire[0]); }
            }
        }
        for (let each of node.customClass.execOutPins) {
            if (each.wire) {
                Delete.deleteWire(each.wire);
                each.wire = null;
            }
        }
    ...
    Delete.deleteHalfWire = (lineClone, originPreOccupied) => {
    ...
    Delete.enableDelete = function (stage, layer) {
