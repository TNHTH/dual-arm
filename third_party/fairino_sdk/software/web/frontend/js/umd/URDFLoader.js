    ...
    */
    ...
    class URDFLoader {
        ...
        load(urdf, onComplete, onProgress, onError) {
            ...
            fetch(urdfPath, this.fetchOptions)
                ...
                .then(data => {
                    ...
                    const model1 = this.parse(data);
                    const model2 = this.parse(data);
                    onComplete(model1, model2);
        ...
        parse(content) {
            ...
            const loadMeshCb = this.loadMeshCb || this.defaultMeshLoader.bind(this);
        ...
        defaultMeshLoader(path, manager, done) {
            ...
            function onProgress(xhr) {
                ...
                if (xhr.lengthComputable) {
                    ...
                    // console.log("model loading: " + Math.round((xhr.loaded / xhr.total * 100), 2) + "%");
            ...
            if (/\.stl$/i.test(path)) {
                ...
                loader.load(path, dae => done(dae.scene), onProgress);
        ...
        loadTool(robot, tool, onComplete, onProgress, onError){

            const manager = this.manager;
            const toolPath = this.manager.resolveURL(tool);

            manager.itemStart(toolPath);
            const newRobot = this.parseTool(robot, toolPath);
            onComplete(newRobot);
            manager.itemEnd(toolPath);

        }

        parseTool(robot, toolUrl) {
            ...
            function progressToolLink(toolUrl) {
                
                let toolObj = new URDFLink();

                loadMeshCb(toolUrl, manager, (obj, err) => {
                    ...
                    if (err) {
                        ...
                        if (obj instanceof THREE.Group) {
                            ...
                            toolObj.name = obj.children[0].name;
                ...
                deleteLastTool(robot);
                robot.tool = null;
                robot.joints['tool'].add(toolObj);
                robot.tool = toolObj;
            ...
            function deleteLastTool(robot) {
                
                let isTool = robot.joints['tool'].children.length;
                
                if (isTool != 0) {
                    
                    let robotTool = robot.joints['tool'].children[0];
                    robotTool.traverse(c => c.dispose && c.dispose());
                    robotTool.parent.remove(robotTool);
                    
                }

            }

            return progressToolLink(toolUrl)
