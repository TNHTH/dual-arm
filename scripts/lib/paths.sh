...
dual_arm_find_repo_root() {
  ...
  while [[ "${start_dir}" != "/" ]]; do
    if [[ -f "${start_dir}/build_workspace.sh" && -d "${start_dir}/packages" ]]; then
      printf '%s\n' "${start_dir}"
      return 0
    fi
...
dual_arm_export_paths() {
  ...
  export DUAL_ARM_SHARED_ROOT="${DUAL_ARM_SHARED_ROOT:-${DUAL_ARM_REPO_ROOT}/.workspace/runtime/shared}"
