...
- 控制台 acceptance 入口已回归：
  - 启动 API：`ros2 run competition_console_api competition_console_api_node.py --ros-args -p host:=127.0.0.1 -p port:=18080 -p allow_hardware_bringup:=false -p api_token:=workspace-camera-smoke`
  - 调用：`curl -X POST -H 'x-dual-arm-token: workspace-camera-smoke' http://127.0.0.1:18080/api/acceptance/run/camera_frames`
