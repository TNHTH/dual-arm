...
本轮完成双臂 no-motion 连接检测和双相机瓶盖单点深度采样。左右机械臂网络、`8080` TCP 端口、`robo_ctrl` 只读状态发布均通过；左右 `/robot_state` 约 `4.996 Hz`，两臂均为 `motion_done=true`、`error_code=0`。左右 Orbbec 已轮流采集 RGB 与 Z16 深度，瓶盖像素点、ROI 深度统计、相机坐标点和截图均已写入 `local execution notes*`。
...
/usr/bin/python3 packages/tools/tools/scripts/cap_depth_alignment_probe.py capture \
  ...
  --output-dir local execution notes \
...
- Overlay：`local execution notes`
- JSON：`local execution notes`
...
- 左 RGB：`local execution notes`
- 左 depth raw：`local execution notes`
- 左 depth 可视化：`local execution notes`
- 左 capture JSON：`local execution notes`
- 右 RGB：`local execution notes`
- 右 depth raw：`local execution notes`
- 右 depth 可视化：`local execution notes`
- 右 capture JSON：`local execution notes`
...
- 左 overlay：`local execution notes`
- 左 analysis JSON：`local execution notes`
...
- 右 overlay：`local execution notes`
- 右 analysis JSON：`local execution notes`
