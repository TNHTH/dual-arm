...
std::string env_or_default(const char* env_name, const std::string& fallback) {
...
std::string resolve_detector_asset(const std::string& relative_path) {
    try {
        const auto share_dir = ament_index_cpp::get_package_share_directory("detector");
        const auto candidate = std::filesystem::path(share_dir) / relative_path;
...
#ifdef USING_YOLOV8
    return env_or_default("DETECTOR_MODEL_PATH", resolve_detector_asset("models/yolov8/best_l.engine"));
#endif
#ifdef USING_LW_DETR
    return env_or_default("DETECTOR_MODEL_PATH", resolve_detector_asset("models/lw_detr/inference_model.engine"));
#endif
    return "";
}
...
#ifdef USING_YOLOV8
    return env_or_default("DETECTOR_PLUGIN_PATH", resolve_detector_asset("models/yolov8/libmyplugins.so"));
#endif
#ifdef USING_LW_DETR
    return env_or_default("DETECTOR_PLUGIN_PATH", resolve_detector_asset("models/lw_detr/layernorm_plugin.so"));
#endif
    return "";
}
...
// 声明参数
    ...
    this->declare_parameter("plugin_path", default_plugin_path());
#endif
#ifdef USING_LW_DETR
    this->declare_parameter("model_path", default_model_path());
    this->declare_parameter("plugin_path", default_plugin_path());
...
// 初始化YOLOv8推理对象
    ...
    if (model_path_.empty()) {
        throw std::runtime_error("未提供 detector 模型路径，请通过参数 model_path 或环境变量 DETECTOR_MODEL_PATH 指定。");
    }
    ...
    try {
        ...
        if (!boxes.empty()) {
            ...
            for (size_t i = 0; i < img_boxes.size() / 4; ++i) {
                ...
                oss << "Class: " << img_class_ids[i] << " (" << std::fixed << std::setprecision(2) << img_scores[i]
                    << ")";
        ...
        if (detections_msg->results.empty()) {
            ...
            RCLCPP_DEBUG(this->get_logger(), "检测到 %zu 个目标", detections_msg->results.size());
