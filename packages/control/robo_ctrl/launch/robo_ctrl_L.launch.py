...
def generate_launch_description():
    ...
    gripper_debug = LaunchConfiguration("gripper_debug")
    gripper_default_slave_id = LaunchConfiguration("gripper_default_slave_id")
    gripper_disable_on_shutdown = LaunchConfiguration("gripper_disable_on_shutdown")
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument("gripper_port", default_value="auto", description="夹爪串口路径，auto 表示自动选择"),
            DeclareLaunchArgument("gripper_debug", default_value="false", description="是否输出夹爪调试日志"),
            DeclareLaunchArgument("gripper_default_slave_id", default_value="9", description="默认夹爪 Modbus 从站 ID"),
            DeclareLaunchArgument(
                "gripper_disable_on_shutdown",
                default_value="false",
                description="节点退出时是否发送夹爪禁用命令",
            ),
            Node(
                package="robo_ctrl",
                ...
                parameters=[
                    {
                        "port": gripper_port,
                        "debug": gripper_debug,
                        "default_slave_id": gripper_default_slave_id,
                        "disable_on_shutdown": gripper_disable_on_shutdown,
                    }
                ],
                output="screen",
            ),
