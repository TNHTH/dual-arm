...
private:
    ...
    bool write_single_register_16(uint16_t register_addr, uint16_t value) {
        ...
        crc = crc16(expected.data(), expected.size());
...
public:
    ...
    bool enable() {
        // 官方手册要求上电后先清使能，再置位使能。
        write_single_register_16(WRITE_REG_START, 0x0000);
        std::this_thread::sleep_for(std::chrono::milliseconds(50));

        if (!write_single_register_16(WRITE_REG_START, 0x0001)) {
            return false;
        }
        ...
        while (std::chrono::steady_clock::now() - start < std::chrono::seconds(8)) {
            ...
            if (!status.empty()) {
                auto status_bits = parse_status_bits(static_cast<uint8_t>(status[0]));
                if (status_bits.gact && status_bits.gsta == 3) {
                    return true;
                }
            }
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        }
        std::cerr << "Error: Activation did not complete within timeout." << std::endl;
        return false;
    }
    ...
    bool disable() {
        return write_single_register_16(WRITE_REG_START, 0x0000);
    }
