...
std::vector<std::string> sorted_matching_paths(const fs::path& directory, const std::string& prefix) {
...
std::string resolve_serial_port(const std::string& requested_port) {
    if (!requested_port.empty() && requested_port != "auto") {
        return requested_port;
    }

    std::vector<std::string> candidates;
    auto by_id_ports = sorted_matching_paths("/dev/serial/by-id", "");
    candidates.insert(candidates.end(), by_id_ports.begin(), by_id_ports.end());

    auto usb_ports = sorted_matching_paths("/dev", "ttyUSB");
    candidates.insert(candidates.end(), usb_ports.begin(), usb_ports.end());

    auto acm_ports = sorted_matching_paths("/dev", "ttyACM");
    candidates.insert(candidates.end(), acm_ports.begin(), acm_ports.end());

    if (candidates.empty()) {
        throw std::runtime_error("未找到可用串口设备，请检查 /dev/serial/by-id、/dev/ttyUSB* 或 /dev/ttyACM*");
    }
...
public:
    EPG50GripperNode(): Node("epg50_gripper_node") {
        // 声明参数
        this->declare_parameter("port", "auto");
        ...
        this->declare_parameter("disable_on_shutdown", false);

        // 获取参数
        std::string requested_port = this->get_parameter("port").as_string();
        ...
        disable_on_shutdown_     = this->get_parameter("disable_on_shutdown").as_bool();
        const std::string port   = resolve_serial_port(requested_port);

        RCLCPP_INFO(
            this->get_logger(),
            "初始化EPG50夹爪, 请求端口: %s, 实际端口: %s, 默认从站ID: 0x%02X, 状态更新频率: %.1f Hz, 命令超时: %.1f s, 退出时禁用: %s",
    ...
    ~EPG50GripperNode() {
        ...
        // 默认不在关闭时发送控制命令，避免调试时产生额外动作。
        if (gripper_ && disable_on_shutdown_) {
            try {
                if (!gripper_->disable()) {
                    RCLCPP_WARN(this->get_logger(), "节点退出时夹爪禁用失败");
                }
            } catch (const std::exception& e) {
                RCLCPP_WARN(this->get_logger(), "节点退出时夹爪禁用异常: %s", e.what());
            }
        }
    }
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::fprintf(stderr, "epg50_gripper_node fatal: %s\n", e.what());
        exit_code = 1;
    }
    rclcpp::shutdown();
    return exit_code;
